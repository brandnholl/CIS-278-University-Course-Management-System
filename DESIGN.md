...
- `std::list` was the other option allowed by the spec. It was rejected because
  list does not block duplicates on its own, and membership checks are O(n).
  With set, the "no duplicate enrollments" rule is enforced by the container
  itself rather than by extra application code.
